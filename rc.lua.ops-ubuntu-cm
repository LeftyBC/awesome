...
tags = {}
tags[1] = {
        names = { "t1", "t2", "t3", "t4", "t5", "virt", "comms", "skype", "home" },
        layout = { layouts[2], layouts[2], layouts[2], layouts[2], layouts[2], layouts[2], layouts[2], layouts[10], layouts[10] }
}
tags[2] = {
        names = { "web", "t2", "t3", "t4", "t5", "t6", "t7", "music", "mail" },
        layout = { layouts[10], layouts[2], layouts[2], layouts[2], layouts[2], layouts[2], layouts[2], layouts[10], layouts[10] }
}
...
awful.rules.rules = {
    ...
    { rule = { class = "banshee" }, properties = { tag = tags[2][8] } },
...
run_once('dbus-launch',nil,1)
run_once('gnome-settings-daemon',nil,1)
run_once('systemd',nil,1)
run_once('gnome-power-manager',nil,1)
run_once('gnome-sound-applet',nil,1)
run_once('gnome-volume-control-applet',nil,1)
run_once('gnome-keyring-daemon','--start --components=ssh,keyring,pkcs11',1)
--run_once('skype',nil,2)
--run_once('empathy',nil,2)
run_once('ssh-add','~/.ssh/id_rsa ~/.ssh/id_rsa-AIRG',1)
run_once('indicator-cpufreq',nil,1)
run_once('nm-applet',nil,1)
run_once('evolution-alarm-notify',nil,1)
run_once('indicator-session-service',nil,1)
run_once('indicator-sound-service',nil,1)
--run_once('banshee',nil,1)

-- Daemons
run_once('xscreensaver','-nosplash',1)
run_once('davmail',nil,1)
run_once('dropbox','start',1)
...
awful.util.spawn_with_shell('date >> ~/awesome_startups')
